  ...
  group('Project', () {
    ...
    group('ensure ready for platform-specific tooling', () {
      ...
      _testInMemory('Android project not on v2 embedding exits', () async {
        ...
        await expectToolExitLater(Future<dynamic>.sync(() => project.checkForDeprecation(deprecationBehavior: DeprecationBehavior.exit)),
      ...
      _testInMemory('creates iOS pod in module', () async {
        ...
        final Directory pluginRegistrantClasses = flutter.childDirectory('FlutterPluginRegistrant').childDirectory('Classes');
...
''';
    ...
    group('product bundle identifier', () {
      ...
      testWithMocks('fails with no flavor and defined schemes', () async {
        ...
        await expectToolExitLater(project.ios.productBundleIdentifier(null),
      ...
      testWithMocks('fails with flavor and default schemes', () async {
        ...
        await expectToolExitLater(project.ios.productBundleIdentifier(buildInfo),
  ...
  group('watch companion', () {
    ...
    group('with bundle identifier', () {
      setUp(() {
        ...
        mockXcodeProjectInterpreter.xcodeProjectInfo =
            XcodeProjectInfo(<String>['Runner', 'WatchTarget'], <String>[], <String>['Runner', 'WatchScheme'], logger);
      });
...
''');
        const XcodeProjectBuildContext buildContext = XcodeProjectBuildContext(scheme: 'Runner', deviceId: '123');
        mockXcodeProjectInterpreter.buildSettingsByBuildContext[buildContext] = <String, String>{
          'PRODUCT_BUNDLE_IDENTIFIER': 'io.flutter.someProject',
        };
...
Future<FlutterProject> someProject({
  ...
  final Directory androidDirectory = directory.childDirectory('android')..createSync(recursive: true);
  androidDirectory.childFile('AndroidManifest.xml').writeAsStringSync(androidManifestOverride ?? '<manifest></manifest>');
...
void _testInMemory(String description, Future<void> Function() testMethod) {
  ...
  transfer(
      ...
      testFileSystem);
  transfer(globals.fs.directory(Cache.flutterRoot).childDirectory('packages').childDirectory('flutter_tools').childDirectory('templates'),
      testFileSystem);
